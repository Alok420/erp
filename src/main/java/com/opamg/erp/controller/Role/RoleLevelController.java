...
import com.opamg.erp.controller.Role.*;
...
public class RoleLevelController {
   ...
   @GetMapping(value = "Role/settings/levels")
   public ModelAndView level(HttpServletRequest request) {
      ...
      modelAndView.setViewName("role/RoleLevelSetting");
      return modelAndView;
   }

   @PostMapping("role/level/create")

   ModelAndView saveLevel(@Valid RoleLevel level, BindingResult br, ModelMap map, HttpServletRequest req) {
      ModelAndView modelAndView = new ModelAndView();
      try {
	if (br.hasErrors()) {
	   modelAndView.clear();
   ...
   @GetMapping(value = "role/level/delete")
   ...
   @GetMapping("role/level/changeLevel")
   @ResponseBody
   ...
   @GetMapping(value = "role/settings/getlevels/", produces = MediaType.APPLICATION_JSON_VALUE)
   public List<RoleLevel> all(@RequestParam Long id) {

      List<RoleLevel> li = levelService.FindLevelByMain(mainService.getMainRepository().findById(id).get());
      return li;
   }

   @PostMapping(value = "role/level/all")
   public List<RoleLevel> allLevel(HttpServletRequest request) {
      long mainid = Long.parseLong(request.getParameter("id"));
      List<RoleLevel> li = levelService.FindLevelByMain(mainService.getMainRepository().findById(mainid).get());
      return li;
   }
