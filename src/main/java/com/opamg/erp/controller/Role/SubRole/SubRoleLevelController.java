...
public class SubRoleLevelController {
  ...
  SubRoleMainService mainService;
   ...
   @GetMapping(value = "subrole/settings/levels")
  public ModelAndView level(HttpServletRequest request) {
    ...
    modelAndView.addObject("SubCharts", mainService.findAllMain());
     modelAndView.setViewName("subrole/RoleLevelSetting");
    return modelAndView;
  }

   @PostMapping("subrole/level/create")

  ModelAndView saveLevel(@Valid SubRoleLevel level, BindingResult br, ModelMap map, HttpServletRequest req) {
    ModelAndView modelAndView = new ModelAndView();
    try {
      if (br.hasErrors()) {
        modelAndView.clear();
   ...
   @GetMapping(value = "subrole/level/delete")
  ModelAndView deleteLevel(@RequestParam long id, HttpServletRequest req) {
   ...
   @GetMapping("subrole/level/changeLevel")
  @ResponseBody
  String changeLevel(@RequestParam long id,@RequestParam String module, HttpServletRequest request) {
   ...
   @GetMapping(value = "subrole/settings/getlevels/", produces = MediaType.APPLICATION_JSON_VALUE)
