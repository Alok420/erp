...
@Controller
@RestController
public class RootLevelFormFieldController {
   ...
   @PostMapping("root/level-form-field/create")
   ModelAndView saveLevelFormFields(@Valid RootLevelFormField levelFormField, BindingResult br, ModelMap map, HttpServletRequest req) {
   ...
   @GetMapping(value = "root/level-form-field/delete")
   ...
   @PostMapping(value = "root/field/all")
   public List<RootLevelForm> allField(HttpServletRequest request) {
      ...
      RootLevelForm levelForm = (RootLevelForm) formli.get(0);
      if (levelForm != null) {
	List<RootLevelForm> li = levelFormFieldService.findByLevelForm(levelForm);
	return li;
      }
      return null;
   }
