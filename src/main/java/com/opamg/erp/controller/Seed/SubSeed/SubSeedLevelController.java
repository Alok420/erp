...
@Controller
@RestController
public class SubSeedLevelController {
  ...
  SubSeedMainService mainService;
   ...
   @GetMapping(value = "subseed/settings/levels")
  public ModelAndView level(HttpServletRequest request) {
    ...
    modelAndView.addObject("SubCharts", mainService.findAllMain());
     modelAndView.setViewName("subseed/SeedLevelSetting");
    return modelAndView;
  }

   @PostMapping("subseed/level/create")

  ModelAndView saveLevel(@Valid SubSeedLevel level, BindingResult br, ModelMap map, HttpServletRequest req) {
    ModelAndView modelAndView = new ModelAndView();
    try {
      if (br.hasErrors()) {
        modelAndView.clear();
   ...
   @GetMapping(value = "subseed/level/delete")
  ModelAndView deleteLevel(@RequestParam long id, HttpServletRequest req) {
   ...
   @GetMapping("subseed/level/changeLevel")
  @ResponseBody
  String changeLevel(@RequestParam long id,@RequestParam String module, HttpServletRequest request) {
   ...
   @GetMapping(value = "subseed/settings/getlevels/", produces = MediaType.APPLICATION_JSON_VALUE)
