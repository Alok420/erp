...
@Controller
@RestController
public class SubSprigLevelController {
  ...
  @GetMapping(value = "subsprig/settings/levels")
  public ModelAndView level(HttpServletRequest request) {
    ...
    modelAndView.setViewName("subsprig/SprigLevelSetting");
    return modelAndView;
  }

  @PostMapping("subsprig/level/create")

  ModelAndView saveLevel(@Valid SubSprigLevel level, BindingResult br, ModelMap map, HttpServletRequest req) {
    ModelAndView modelAndView = new ModelAndView();
    try {
      if (br.hasErrors()) {
        modelAndView.clear();
  ...
  @GetMapping(value = "subsprig/level/delete")
  ...
  @GetMapping("subsprig/level/changeLevel")
  @ResponseBody
  ...
  @GetMapping(value = "subsprig/settings/getlevels/",produces = MediaType.APPLICATION_JSON_VALUE)
