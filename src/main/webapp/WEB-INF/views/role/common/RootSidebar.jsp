    ...
    <c:forEach items="${MainService.findAllMain()}" var="main">
      ...
      <c:choose>
        <c:when test = "${param.id ==main.id}">
          <a style="" href="/role/Dashboard?id=${main.id}&subid=${param.subid}"><li style="list-style: none; color: white; padding: 5px;background-color: white; color: black;"> ${main.name} </li> </a>
        </c:when>
        <c:otherwise>
          <a href="/role/Dashboard?id=${main.id}&subid=${param.subid}"><li style="list-style: none; color: white; padding: 5px"> ${main.name} </li> </a>
        </c:otherwise>
      </c:choose>
    </c:forEach>
    <li>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form action="../role/main/create" onsubmit="return validaterRootChartForm()"  method="POST" enctype="multipart/form-data">
