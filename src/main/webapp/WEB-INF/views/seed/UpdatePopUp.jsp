	...
	<div class="modal-body">
	   <div class="container-fluid" >
		...
		<div id="formBox"  style="width: 100%;">
		   ...
		   <form method="POST" action="" id="seedfdupdate${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}" enctype="multipart/form-data">
			...
			<c:forEach items="${LevelFormFieldService.getRepository().findByLevelForm(form)}" var="field" varStatus="">
		      ...
		      <div class="row form-group">
			<div class="col-lg-3">
			   <input type="button" onclick="updateSeedData('seedfdupdate${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}', 'saveSeedFormDataupdatebox${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}')"  value="Save" class="btn btn-block btn-outline-success">

			</div>
			<div class="col-lg-1">
			   <div style="color: black;" id="saveSeedFormDataupdatebox${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}"></div>
			</div>
			<div class="col-lg-8">
						...
						<div class="row" style="background-color: white;">
						   ...
						   <div id="formBox"  style="padding: 0px; width: 100%;">
							...
							<form method="POST" action="" id="formdataupdate${form.id}${sublevel.id}${subform.id}${row.id}">
							   <div class="row form-group">
							      <div class="col-lg-12">
								<input type="hidden" class="seedFormDataOptionList2" name="seedFormData" value="${row.id}">
							      </div>
							   </div>
							   <div class="row form-group">
							   ...
							   <div class="row form-group">
							      <div class="col-lg-3">
								<input type="button" onclick="saveSubSeedData2('formdataupdate${form.id}${sublevel.id}${subform.id}${row.id}')" value="Save" class="btn btn-block btn-outline-success">
							      </div>
							      <div class="col-lg-3">
								<div id="saveSubSeedFormData2" style="color: black;"></div>
							      </div>
							      <div class="col-lg-6">
							...
							<div style="background-color: #003366; padding: 5px;border-radius: 5px;">
							   <table id="subtable${sublevel.id}${subform.id}${row.id}" class="display nowrap" style="width:100%;color: black;" >
								...
								<th>
							      ...
							      <tbody>
							      <c:forEach items="${SubFormDataService.getRepository().findBySeedFormData(row)}" var="subrow" varStatus="subrindex">
								<tr>
								   <td>${subrow.id}</td>
								<c:forEach items="${SubLevelFormFieldService.getRepository().findByLevelForm(subform)}" var="suballField" varStatus="subj">
								   <td>
								...
								<td>${subrow.seedFormData.id}</td>
								<td>
								   <a href="../subseed/level-form-data/delete/?id=${subrow.id}" onclick="return confirm('Are you sure? you want to delete this record')" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
