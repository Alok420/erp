      ...
      <c:forEach items="${SubMainService.findAllBySeedLevelForm(form)}" var="submain">
         <c:choose>
            <c:when test = "${param.subid ==submain.id}">
               <a style="" href="/seed/Dashboard?id=${param.id}&subid=${submain.id}"><li style="width: 250px; list-style: none; color: white; padding: 5px;background-color: white; color: black;"> ${submain.name} </li> </a>

            </c:when>
            <c:otherwise>
               <a href="/seed/Dashboard?id=${param.id}&subid=${submain.id}"><li style="list-style: none; color: white; padding: 5px"> ${submain.name} </li> </a>
            </c:otherwise>
         </c:choose>
      </c:forEach>
      <li>
                     ...
                     <form action="../subseed/main/create" onsubmit="return validaterRootChartForm()"  method="POST" enctype="multipart/form-data">
                        <table align="center" width="100%">
                           <tr>
                              ...
                              <td>
                                 <input type="hidden" name="seedLevelForm" value="${form.id}">
                                 <label>&nbsp;&nbsp;&nbsp;</label>
