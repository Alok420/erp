		...
		<div class="row">
		   <div class="col-sm-11">
			...
			<div id="formBox"  style="width: 100%; ">
			   ...
			   <form method="POST" action="" id="mybranchfd${form.id}${level.id}" enctype="multipart/form-data">
				...
				<c:forEach items="${LevelFormFieldService.getRepository().findByLevelForm(form)}" var="field" varStatus="">
				   <c:choose>
					...
					<div class="col-lg-2 mb-2">
					   <strong class="pl-2" style="font-size: 12px;">${field.name}</strong>
					   <select id="mybranchfd${form.id}${level.id}_dropdown_field_id_${field.id}" onchange="getDropDownData(this, '${field.default_value}',${form.id},${field.id}, 'mybranchfd${form.id}${level.id}_dropdown_field_id_')" class="bg-focus form-control" data="${field.default_value}" required name="${field.name}" placeholder="${field.placeholder}" autocomplete="off" style="height: 35px;">
					      <option>Select any one</option>
					      <c:choose>
						<c:when test="${relational_ids[0] eq 'MyBranch'}">
			      ...
			      <div class="row form-group">
				<div class="col-lg-3">
				   <input type="button" onclick="saveMyBranchData('mybranchfd${form.id}${level.id}')"  value="Save" class="btn btn-block btn-outline-success">
				</div>
				<div class="col-lg-1">
				   <div style="color: black;" id="saveMyBranchFormData">
							...
							<div class="row" style="">
							   ...
							   <div id="formBox"  style="padding: 0px; width: 100%;">
								...
								<form method="POST" action="" id="formdata${form.id}${sublevel.id}" style="border:thin solid #003366; margin-bottom: 10px;padding: 5px;border-radius: 5px;">
								   <div class="row form-group">
								      <div class="col-lg-12">
									<input type="hidden" class="mybranchFormDataOptionList" name="formData" id="mybranchFormDataOptionList">
									<%--    <select class="form-control mybranchFormDataOptionList" required name="mybranchFormData" id="mybranchFormDataOptionList">
								   ...
								   <div class="row form-group">
									...
									<c:choose>
									   ...
									   <c:when test="${subfield.type eq 'dropdown' and subfield.default_value_type eq 'select_parent'}">
									      ...
									      <div class="col-lg-2 mb-2">
										<strong class="pl-2" style="font-size: 12px;">${subfield.name}</strong>
										<select id="mybranchfd${subform.id}${sublevel.id}_dropdown_field_id_${subfield.id}" onchange="getSubMyBranchDropDownData(this, '${subfield.default_value}',${subform.id},${subfield.id}, 'mybranchfd${subform.id}${sublevel.id}_dropdown_field_id_')" class="bg-focus form-control" data="${subfield.default_value}" required name="${subfield.name}" placeholder="${subfield.placeholder}" autocomplete="off" style="height: 35px;">
										   <option>Select any one</option>
										   <c:choose>
										      <c:when test="${relational_ids[0] eq 'MyBranch'}">
											<c:forEach items="${MyBranchFormDataService.findByLevelForm(MyBranchLevelForm.findByLevel(MyBranchLevel.findOneById(relational_ids[2])).get(0))}" var="obj" varStatus="i">
								   ...
								   <div class="row form-group">
								      <div class="col-lg-3">
									<input type="button" onclick="saveSubMyBranchData('formdata' +${form.id}${sublevel.id})" value="Save" class="btn btn-block btn-outline-success">
								      </div>
								      <div class="col-lg-3" >
									<div id="saveSubMyBranchFormData" style="color: black;"></div>
								      </div>
								      <div class="col-lg-6" >
