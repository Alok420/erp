	...
	<div class="modal-body">
	   <div class="container-fluid" >
		...
		<div id="formBox"  style="width: 100%;">
		   ...
		   <form method="POST" action="" id="mybranchfdupdate${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}" enctype="multipart/form-data">
			...
			<c:forEach items="${LevelFormFieldService.getRepository().findByLevelForm(form)}" var="field" varStatus="">
		      ...
		      <div class="row form-group">
			<div class="col-lg-3">
			   <input type="button" onclick="updateMyBranchData('mybranchfdupdate${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}', 'saveMyBranchFormDataupdatebox${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}')"  value="Save" class="btn btn-block btn-outline-success">

			</div>
			<div class="col-lg-1">
			   <div style="color: black;" id="saveMyBranchFormDataupdatebox${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}"></div>
			</div>
			<div class="col-lg-8">
		   ...
		   <div class="modal" id="${fn:replace(form.level.chart.name,' ', '_')}${form.level.id}_${i.index}${row.id}">
			...
			<div class="modal-content">
			   ...
			   <div class="modal-body">
			      <form action="../mybranch/level-form-field/create"  method="POST" enctype="multipart/form-data">
				   ...
				   <ul class="nav nav-tabs">
					...
					<div id="subaddlevel${submain.id}${form.id}${sublastlevel.id}${row.id}" class="modal fade" role="dialog">
					   <div class="modal-dialog">
						...
						<div class="modal-body">
						   <form action="../submybranch/level/create" onsubmit="return validateLevelForm()" method="POST" enctype="multipart/form-data">
					   ...
					   <div>
						...
						<div id="subaddform${form.id}${sublevel.id}${row.id}" class="modal fade" role="dialog">
						   <div class="modal-dialog">
							...
							<div class="modal-body">
							   <form onsubmit="validateLevelForm()" action="../submybranch/level-form/create" method="POST" enctype="multipart/form-data">
						...
						<div class="row" style="background-color: white;">
						   ...
						   <div id="formBox"  style="padding: 0px; width: 100%;">
							...
							<form method="POST" action="" id="formdataupdate${form.id}${sublevel.id}${subform.id}${row.id}">
							   <div class="row form-group">
							      <div class="col-lg-12">
								<input type="hidden" class="mybranchFormDataOptionList2" name="formData" value="${row.id}">
							      </div>
							   </div>
							   <div class="row form-group">
							   ...
							   <div class="row form-group">
							      <div class="col-lg-3">
								<input type="button" onclick="saveSubMyBranchData2('formdataupdate${form.id}${sublevel.id}${subform.id}${row.id}')" value="Save" class="btn btn-block btn-outline-success">
							      </div>
							      <div class="col-lg-3">
								<div id="saveSubMyBranchFormData2" style="color: black;"></div>
							      </div>
							      <div class="col-lg-6">
							...
							<div class="modal" id="sub${fn:replace(subform.level.chart.name,' ', '_')}${subform.level.id}_${i.index}${row.id}">
							   <div class="modal-dialog">
								...
								<div class="modal-body">
								   <form action="../submybranch/level-form-field/create"  method="POST" enctype="multipart/form-data">
							...
							<div style="background-color: #003366; padding: 5px;border-radius: 5px;">
							   <table id="subtable${sublevel.id}${subform.id}${row.id}" class="display nowrap" style="width:100%;color: black;" >
								...
								<th>
							      ...
							      <tbody>
							      <c:forEach items="${SubFormDataService.getRepository().findByFormData(row)}" var="subrow" varStatus="subrindex">
								<tr>
								   <td>${subrow.id}</td>
								<c:forEach items="${SubLevelFormFieldService.getRepository().findByLevelForm(subform)}" var="suballField" varStatus="subj">
								   <td>
								...
								<td>${subrow.formData.id}</td>
								<td>
								   <a href="../submybranch/level-form-data/delete/?id=${subrow.id}" onclick="return confirm('Are you sure? you want to delete this record')" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
