      ...
      <c:forEach items="${SubMainService.findAllByTrunkLevelForm(form)}" var="submain">
         <c:choose>
            <c:when test = "${param.subid ==submain.id}">
               <a style="" href="/trunk/Dashboard?id=${param.id}&subid=${submain.id}"><li style="width: 250px; list-style: none; color: white; padding: 5px;background-color: white; color: black;"> ${submain.name} </li> </a>

            </c:when>
            <c:otherwise>
               <a href="/trunk/Dashboard?id=${param.id}&subid=${submain.id}"><li style="list-style: none; color: white; padding: 5px"> ${submain.name} </li> </a>
            </c:otherwise>
         </c:choose>
      </c:forEach>
      <li>
                     ...
                     <form action="../subtrunk/main/create" onsubmit="return validaterRootChartForm()"  method="POST" enctype="multipart/form-data">
                        <table align="center" width="100%">
                           <tr>
                              ...
                              <td>
                                 <input type="hidden" name="trunkLevelForm" value="${form.id}">
                                 <label>&nbsp;&nbsp;&nbsp;</label>
